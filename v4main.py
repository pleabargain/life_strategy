...
# 53, in <module>
#     cursor.execute(f'''
# sqlite3.OperationalError: 35 values for 34 columns
...
def get_user_inputs():
    ...
    for value in values:
        ...
        inputs.append((value.replace(' ', '_').lower(), score, quadrant))
...
columns_definition = ', '.join([f"{value.replace(' ', '_').lower()}_score INTEGER, {value.replace(' ', '_').lower()}_quadrant INTEGER" for value in values])
columns_names = ', '.join([f"{value.replace(' ', '_').lower()}_score, {value.replace(' ', '_').lower()}_quadrant" for value in values])
...
cursor.execute(f'''
    INSERT INTO user_input (id, user_name, {columns_names})
    VALUES (?, {placeholders})
''', [input_id] + data)
...
for value, score, quadrant in user_inputs:
    ...
    ax.scatter(x, y, s=20 * score, alpha=0.5, label=value.replace('_', ' '))
